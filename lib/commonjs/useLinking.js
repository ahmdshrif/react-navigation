...
let isNewEntriesPushed = false;
let items = [];

const createMemoryHistory = () => {
  let index = 0; // Pending callbacks for `history.go(n)`
  ...
  React.useEffect(() => {
    ...
    if (ref.current) {
      ...
      if (state) {
        ...
        let allNewEntries = new Array(); //Pushing new entry in history

        if (state.routes.length > 1 && items.length && !isNewEntriesPushed) {
          state.routes.map(historyRoute => {
            let updatedRoute = [];
            ...
            const newRoute = (0, _core.findFocusedRoute)(updatedState);
